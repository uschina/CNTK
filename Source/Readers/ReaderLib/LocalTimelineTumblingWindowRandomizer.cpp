...
LocalTimelineTumblingWindowRandomizer::LocalTimelineTumblingWindowRandomizer(
  ...
  m_sweepCount(0)
{
    ...
    m_rng.seed((unsigned long)m_sweepCount + m_seedOffset);
    Microsoft::MSR::CNTK::RandomShuffleMT(m_prefetchedChunkDescriptions, m_rng);
}

void LocalTimelineTumblingWindowRandomizer::RandomizeWindow(size_t sweepCount, size_t chunkPositionOfWindow, size_t sequencePositionInWindow) const
{
...
void LocalTimelineTumblingWindowRandomizer::RandomizeChunks(size_t sweepCount) const
{
    m_prefetchedChunkDescriptions = m_originalChunkDescriptions;
...
void LocalTimelineTumblingWindowRandomizer::Prefetch() const
{
...
void LocalTimelineTumblingWindowRandomizer::RefillSequenceWindow(SequenceWindow& window)
{
...
std::map<std::wstring, size_t> LocalTimelineTumblingWindowRandomizer::GetInnerState()
{
...
void LocalTimelineTumblingWindowRandomizer::SetInnerState(const std::map<std::wstring, size_t>& state)
{
