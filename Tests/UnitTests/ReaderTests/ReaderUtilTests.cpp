...
BOOST_AUTO_TEST_CASE(Index_with_caching)
{
    ...
    auto index = TextInputIndexBuilder(f1).SetCachingEnabled(true).Build();
    ...
    FILE* dummy = nullptr;
    auto cachedIndex = TextInputIndexBuilder(FileWrapper(filename, dummy)).SetCachingEnabled(true).Build();

    CheckIdentical(index, cachedIndex);
}

BOOST_AUTO_TEST_CASE(Index_1MB_with_caching_check_perf)
{
    ...
    for (int i = 0; i < 10; i++) 
    {
        ...
        auto index = TextInputIndexBuilder(f1).SetCachingEnabled(true).SetChunkSize(g_1MB >> 4).Build();
        ...
        start = GetTickCount();
        FILE* dummy = nullptr;
        auto cachedIndex = TextInputIndexBuilder(FileWrapper(filename, dummy))
            .SetCachingEnabled(true).SetChunkSize(g_1MB >> 4).Build();
        end = GetTickCount();
